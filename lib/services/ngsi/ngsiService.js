...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    ...
    for (var i = 0; i < attributes.length; i++) {
        ...
        if (existsAttribute(attribute['name'], typeInformation)) {
            payload[getAttributeName(attribute['name'], typeInformation)] = {"value": attribute['value'], "type": attribute['type']};
        }
    }

    payload = JSON.stringify(payload);

    var headers = createHeaders(typeInformation);

    var options = createOptions(entityName, headers, payload);

    request(options, generateNGSIOperationHandler('update', entityName, typeInformation, token, options, callback));

    function getAttributeName(id, typeInformation) {
        var active = typeInformation['active'];
        var foundIt = false;
        var ret = id;
        for (var i=0; i<active.length && !foundIt; i++) {
            if(active[i]['object_id'] == id) {
                ret = active[i]['name'];
                foundIt = true;
            }
        }
    ...
    function existsAttribute(id, typeInformation) {
        var active = typeInformation['active'];
        var foundIt = false;
        for (var i=0; i<active.length && !foundIt; i++) {
            if(active[i]['object_id'] == id) {
    ...
    function createUrl(entityName) {
        return "http://" + config.getConfig().contextBroker.host + ":" + config.getConfig().contextBroker.port + "/v2/entities/" + entityName + "/attrs";
    }

    function createHeaders(typeInformation) {
        return {
            'cache-control': 'no-cache',
            'content-type': 'application/json',
            'fiware-service': typeInformation['service'],
            'fiware-servicepath': typeInformation['subservice']
        };
    }

    function createOptions(entityName, headers, payload) {
        return {
            'url': createUrl(entityName),
            'method': 'POST',
            'headers': headers,
            'body': payload
        };
